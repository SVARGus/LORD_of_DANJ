...
void Hero::upExp(int exp) // Получение опыта и если достигнут или превышен уровень scalExpUp и пересчитывать уровень scalExpUp через метод recalculateCharacteristic
{
	scalExp += exp;
	if ((scalExp % scalExpUp) == 1)
	{
		cout << "Поздравляю ваш уровень повышен до " << lvl++ << endl;
		scalExp -= scalExpUp;
